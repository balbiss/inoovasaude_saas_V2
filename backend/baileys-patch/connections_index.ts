    ...
    async ({ params, query }) => {
      ...
      try {
        ...
        const profilePictureUrl = await Promise.race([
          baileys.profilePictureUrl(phoneNumber, jid, type),
          new Promise<string | undefined>((_, reject) =>
            setTimeout(() => reject(new Error("profile-picture-timeout")), TIMEOUT_MS),
          ),
        ]);
        ...
        // Return null for any error (not-found, timeout, disconnected, etc.)
        // instead of throwing which closes the TCP socket without HTTP response
        return new Response(
          JSON.stringify({ data: { jid, profilePictureUrl: null } }),
